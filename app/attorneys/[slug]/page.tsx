/**
 * app/attorneys/[slug]/page.tsx — Attorney bio (design 04-attorney-bio.html).
 * Olive header (photo + intro) → narrative + credentials aside → CTA. Person +
 * Breadcrumb JSON-LD. Statically generated for each attorney in team.ts.
 *
 * VERIFY before launch: credentials, bar admissions, associations.
 */
...
import { attorneys, getMember } from "@/content/team";
...
export default async function BioPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
        ...
        <Container>
          <div className="bio-head">
            ...
            <div>
              ...
              {intro && <p className="lead" style={{ maxWidth: "52ch" }}>{intro}</p>}
      ...
      <CTA heading={`Schedule with ${m.name}`} />
    </>
  );
}
